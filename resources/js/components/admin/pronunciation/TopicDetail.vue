    ...
    async getDetailTopic() {
      try {
        let rs = await baseRequest.get(
          `/admin/detail-topic-pronunciation/${this.param}`
        );
        if (rs.data.status == 200) {
          ...
          let temp = data.question_pronunciation?.map((item) => {
            return {
              id: item.id,
              question: item.question,
              level: item.level,
              type: item.type,
              answers: item.answer_pronunciation.map((itemAns) => {
                return {
              ...
              right_answer: item.right_answer_pronunciation
                ? item.right_answer_pronunciation.answer_id
                : null,
            };
          });
