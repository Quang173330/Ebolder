...
export default {
        ...
        try {
          let result = await baseRequest.post(
            "/admin/save-exam-history-final",
            {
              ...
              result_pronunciation: `${this.arrRightAns.length}/${this.answerData.length}`,
              time: this.timerun,
            }
          );
          if (result.data.status === 200) {
            window.location.href = `${$Api.baseUrl}/exam/result/${this.request.historyId}&v=${this.request.v}&g=${this.request.g}&l=${this.request.l}&s=${this.request.s}&w=${this.request.w}&p=${this.request.p}&r=${this.request.r}&historyId=${this.request.historyId}&examId=${this.request.examId}&exam=true`;
          }
        } catch (err) {
          console.log(err);
        }
  ...
  created() {
    this.topic = {
      ...
      questions: this.data.question_pronunciation.map((pronunciation) => ({
        id: pronunciation.id,
        question: pronunciation.question,
        type: pronunciation.type,
        dataAns: pronunciation.answer_pronunciation.map((item) => ({
          id: item.answer_id,
          text: item.text,
        })),

        right_answer: pronunciation.right_answer_pronunciation
          ? pronunciation.right_answer_pronunciation.answer_id
          : "",
      })),
    };
